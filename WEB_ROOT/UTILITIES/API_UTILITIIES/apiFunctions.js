...
export async function AdvertiserLogin(usernameOrEmail = "", password = "") {
    try {
        const route = API_ROUTES.ADVERTISER.LOGIN;
        const payload = API_PAYLOAD_FACTORY.ADVERTISER.LOGIN(usernameOrEmail, password);
        return await ConsumeApi(route, API_METHOD.POST, payload);
    } catch (e) {
        console.error("Failed to Login Advertiser:", e);
        throw e;
    }
}
